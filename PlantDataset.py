...
class PlantDataset(utils.Dataset):
    ...
    def random_image(self, height, width):
        ...
        keep_ixs = utils.non_max_suppression(np.array(boxes), np.arange(N), 0.5)    # 0.3   0.5
    ...
    def random_image_multiple_plants(self, height, width):
        ### AZ note - this is a special case for 1 plant in the middle, 1 plant on the right and 1 plant on the left
        ### only the middle plant gets a mask
        bg_color = np.array([random.randint(0, 255) for _ in range(3)])
        shapes = []
        boxes = []
        num_plants = random.randint(self.min_plants, self.max_plants)
        ...
        for curr_plant in range(num_plants):
            ...
            if curr_plant == 0:
                ...
                min_x = math.floor(-0.2 * width)
                max_x = math.floor(0.2 * width)
                min_y = math.floor(0.3 * height)
                max_y = math.floor(0.7 * height)
            elif curr_plant == 2:
                min_x = math.floor(0.8 * width)
                max_x = math.floor(1.2 * width)
                min_y = math.floor(0.3 * height)
                max_y = math.floor(0.7 * height)
            ...
            for _ in range(N_curr):
                ...
                prev_angle = angle
                y, x, channels = np.asarray(self.img2[index]).shape
                plant_idx = curr_plant
                shapes.append((shape, location, scale, angle, index, plant_idx))
                boxes.append([location[1], location[0], location[1] + y, location[0] + x])
    ...
    def load_image(self, image_id):
        ...
        for shape, location, scale, angle, index, plant_idx in info['shapes']:
    ...
    def load_mask(self, image_id):
        ...
        mask = np.zeros([info['height'], info['width'], 1], dtype=np.uint8) #AZ plant
        num_valid_masks = 0

        for i, (shape, location, scale, angle, index, plant_idx) in enumerate(info['shapes']):
            if plant_idx == 0:
                ...
                #mask[:, :, i] = temp[:, :] #AZ plant
                mask[:, :, 0] = np.logical_or(mask[:, :, 0], temp[:, :])


        # occlusion = np.logical_not(mask[:, :, -1]).astype(np.uint8)
        # for i in range(count - 2, -1, -1):
        #     mask[:, :, 0] = mask[:, :, 0] * occlusion
        #     occlusion = np.logical_and(occlusion, np.logical_not(mask[:, :, 0]))
